...
export function useAuth(): AuthState {
  ...
  useEffect(() => {
    ...
    const loadFor = async (u: User | null) => {
      if (!u) { if (!active) return; setRoles([]); setProfile(null); setLoading(false); return; }
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((_event, session) => { setUser(session?.user ?? null); setLoading(true); loadFor(session?.user ?? null); });
    return () => { active = false; sub.subscription.unsubscribe(); };
  }, []);

  // Real driver GPS. Permission errors are surfaced instead of being silently swallowed.
  useEffect(() => {
    if (!user || !roles.includes("driver") || profile?.kyc_status !== "approved" || !navigator.geolocation) return;
    const watchId = navigator.geolocation.watchPosition(
      async (position) => {
        const { latitude, longitude, accuracy, heading, speed } = position.coords;
        const { error } = await supabase.from("driver_locations").upsert({
          driver_id: user.id, latitude, longitude, accuracy_m: accuracy ?? null,
          heading_deg: heading ?? null, speed_mps: speed ?? null, updated_at: new Date().toISOString(),
