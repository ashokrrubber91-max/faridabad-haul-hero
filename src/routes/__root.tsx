...
function ErrorComponent({ error, reset }: { error: unknown; reset: () => void }) {
  ...
  useEffect(() => { const normalized = error instanceof Error ? error : new Error(String(error)); reportLovableError(normalized, { boundary: "tanstack_root_error_component" }); }, [error]);
...
function RootComponent() {
  ...
  useEffect(() => {
    ...
    return () => { mounted = false; (window as unknown as { __sbSub?: { unsubscribe: () => void } }).__sbSub?.unsubscribe(); };
  }, [queryClient, router]);
  ...
  return <QueryClientProvider client={queryClient}>
    {isAdmin && <div className="fixed right-3 top-3 z-[100] flex items-center gap-1 rounded-full border bg-background/95 p-1 shadow-lg backdrop-blur"><span className="hidden px-2 text-xs font-semibold text-secondary sm:inline-flex"><UserRound className="mr-1 h-3.5 w-3.5" />{profile?.name || "Admin"}</span><Button size="sm" variant="ghost" className="rounded-full text-destructive" onClick={() => void adminSignOut()}><LogOut className="mr-1 h-3.5 w-3.5" />Logout</Button></div>}
    <Outlet /><Toaster richColors position="top-center" /><OfflineBanner />
  </QueryClientProvider>;
}
