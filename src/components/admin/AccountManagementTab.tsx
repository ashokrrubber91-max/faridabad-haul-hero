    ...
    <div className="space-y-4">
      <section className="surface-card">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Admin-only account creation, access control and password recovery. SMS OTP is not
              required.
            </p>
          </div>
        ...
        <div className="divide-y divide-border">
          {loading && (
            <p className="px-4 py-8 text-center text-sm text-muted-foreground">Loading accounts…</p>
          )}
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {a.lastSignIn
                    ? ` · Last login ${new Date(a.lastSignIn).toLocaleString("en-IN", { dateStyle: "short", timeStyle: "short" })}`
                    : " · Never logged in"}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <Button
                  ...
                  onClick={() => {
                    setResetOpen(a);
                    setResetPasswordValue(makeTempPassword());
                  }}
                >
                ...
                <Button
                  size="sm"
                  variant="outline"
                  disabled={busy || a.roles.includes("admin")}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Create MiniPort account</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={!!resetOpen} onOpenChange={(o) => !o && setResetOpen(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Reset password — {resetOpen?.name || resetOpen?.phone}</DialogTitle>
          </DialogHeader>
          <div>
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            No SMS is sent. Share the new password securely with the account owner.
          </p>
          <DialogFooter>
